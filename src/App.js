...
export default function App() {
  ...
  const generateTopics = async () => {
    ...
    try {
      ...
      const text = await callGroq(`${cat.prompt} 신박하고 흥미로운 유튜브 쇼츠 주제 5개. "우주에서 라면을 끓이면?" 같은 의문형. 줄바꿈으로 구분, 번호없이, 한국어만.`);
  ...
  const generateScript = async () => {
    ...
    try {
      const text = await callGroq(`유튜브 쇼츠 스크립트. 주제: "${topic}". 45~55초(약 220자), 강렬한 첫 후킹, 핵심정보 2~3가지, "구독하면 더 신박한 거 알려줄게요!"로 마무리. 나레이션 텍스트만, 한국어.`);
      setScript(text);
    } catch (e) { setError("API 오류: " + e.message); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#03020a", color: "#fff", fontFamily: "'Segoe UI', sans-serif", position: "relative", overflowX: "hidden" }}>
      <style>{`
        ...
        .btn-primary { background: linear-gradient(135deg,#f97316,#ef4444); color:#fff; border:none; border-radius:14px; padding:14px 20px; font-size:14px; font-weight:700; cursor:pointer; width:100%; transition:.2s; }
        ...
        .topic-item:hover { border-color:rgba(249,115,22,0.4); }
        .topic-item.active { border-color:#f97316; background:rgba(249,115,22,0.12); color:#fff; font-weight:600; }
        ...
        .cat-btn:hover { border-color:rgba(249,115,22,0.4); }
        .cat-btn.active { border-color:#f97316; background:rgba(249,115,22,0.15); color:#f97316; font-weight:700; }
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:480, margin:"0 auto", padding:"0 16px 140px" }}>
        <div style={{ textAlign:"center", padding:"48px 0 28px" }}>
          ...
          <div style={{ fontSize:26, fontWeight:800, background:"linear-gradient(135deg,#f97316,#ef4444,#a78bfa)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", marginBottom:6 }}>Shorts Studio</div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.35)", letterSpacing:3, textTransform:"uppercase" }}>AI 유튜브 쇼츠 자동화</div>
          <div style={{ marginTop:8, display:"inline-block", padding:"4px 12px", borderRadius:20, background:"rgba(249,115,22,0.15)", border:"1px solid rgba(249,115,22,0.3)", fontSize:11, color:"#f97316" }}>⚡ Groq 무료</div>
        </div>

        <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:4, marginBottom:32 }}>
          {STEPS.map((s, i) => (
            <div key={s} style={{ display:"flex", alignItems:"center", gap:4 }}>
              <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color: i < step ? "#f97316" : i === step ? "#fb923c" : "rgba(255,255,255,0.25)", fontWeight: i === step ? 700 : 400 }}>
                <div style={{ width:6, height:6, borderRadius:"50%", background: i < step ? "#f97316" : i === step ? "#fb923c" : "rgba(255,255,255,0.2)" }} />
                {s}
              </div>
        ...
        {step === 1 && (
          <>
            <div className="card">
              <div style={{ fontSize:11, color:"rgba(249,115,22,0.8)", fontWeight:700, letterSpacing:1, marginBottom:8, textTransform:"uppercase" }}>선택한 주제</div>
              <div style={{ fontSize:15, fontWeight:700, color:"#fff", lineHeight:1.6, marginBottom:16 }}>{topic}</div>
            ...
            {script && (
              <div className="card">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(script); setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ background:"none", border:"none", color:"#f97316", fontSize:12, cursor:"pointer" }}>{copied ? "✅ 복사됨" : "📋 복사"}</button>
                </div>
        ...
        {step === 2 && (
          <>
            <div className="card">
              ...
              <div style={{ padding:"14px", borderRadius:12, background:"rgba(249,115,22,0.08)", border:"1px solid rgba(249,115,22,0.2)", marginBottom:10 }}>
                <div style={{ fontWeight:700, color:"#f97316", marginBottom:6, fontSize:13 }}>✅ Edge TTS (완전 무료)</div>
                <div style={{ fontSize:12, color:"rgba(255,255,255,0.5)", lineHeight:1.7 }}>Microsoft 무료 TTS · 한국어 자연스러움</div>
                <div style={{ marginTop:10, padding:"10px 12px", background:"rgba(0,0,0,0.4)", borderRadius:8, fontFamily:"monospace", fontSize:11, color:"#fed7aa", lineHeight:1.8 }}>pip install edge-tts<br/>edge-tts --voice ko-KR-SunHiNeural \<br/>--text "스크립트" --write-media out.mp3</div>
              </div>
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, padding:"12px 16px 28px", background:"rgba(3,2,10,0.96)", borderTop:"1px solid rgba(255,255,255,0.07)", zIndex:100, backdropFilter:"blur(20px)" }}>
        {showApiInput ? (
          <div style={{ display:"flex", gap:8 }}>
            <input type="password" placeholder="Groq API 키 (gsk_...)" value={apiKey} onChange={e => setApiKey(e.target.value)} />
            <button className="btn-primary" style={{ width:"auto", padding:"12px 18px" }} onClick={() => { localStorage.setItem("groq_key", apiKey); setShowApiInput(false); }}>저장</button>
          </div>
        ) : (
          <button className="btn-ghost" style={{ border: apiKey ? "1px solid rgba(249,115,22,0.3)" : "1px solid rgba(255,255,255,0.1)", color: apiKey ? "#f97316" : "rgba(255,255,255,0.7)" }} onClick={() => setShowApiInput(true)}>
            {apiKey ? "⚡ Groq API 키 설정됨 ✅" : "⚡ Groq API 키 입력하기"}
          </button>
        )}
      </div>
    </div>
